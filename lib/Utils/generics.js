...
const generateMessageID = () =>
  "7EPP3LI-" + (0, crypto_1.randomBytes)(6).toString("hex").toUpperCase();
...
const fetchLatestWaWebVersion = async (options = {}) => {
  try {
    ...
    if (!response.ok) {
      throw new Error(
        `Failed to fetch sw.js: ${response.status} ${response.statusText}`,
      );
    }
...
const fetchLatestBaileysVersion = async () => {
  return {
    ...
    isLatest: false, // we didn't check the internet
  };
};
